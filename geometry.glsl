...
//originalOffset is the basic displacement of the original vertex
...
void emitShapeExtrude(int sidesNumber, vec4 extrudeVector, vec4 originalOffset, vec2 scale, bool join) {
	for (int i = 0; i < sidesNumber; ++i) {
		...
		vec4 v1 = gl_in[0].gl_Position + offset1 + originalOffset;
		vec4 v2 = gl_in[0].gl_Position + offset2 + originalOffset;
		vec4 v3 = gl_in[0].gl_Position + originalOffset;
		emitTriangle(v1, v2, v3);
...
void main() {
    //emitShapeExtrude(int(u_time * 0.25) % 4 + 3, vec4(0.0, 0.125, 0.25, .0), vec4(0.0), vec2(0.4), false);
    emitShapeExtrude(3, vec4(0.0, 0.125, 0.25, .0), -gl_in[0].gl_Position + ampersandCurve(gl_in[0].gl_Position.xy), vec2(0.4), true); //alongCurve
